...
export default function PageUsingHooks() {
  ...
  /* `useTreatment` returns the evaluated treatment of the given feature flag name along with the SDK status (`isReady`, `isReadyFromCache`, `isTimedout`, `lastUpdate`, etc).
   * While the SDK is not ready or ready from cache, treatments values are `control`. */
  const { treatment, isReady, isTimedout } = useTreatment({ names: feature_flag_1 });
